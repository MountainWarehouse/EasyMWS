...
namespace EasyMWS.Tests.Processors
{
	public class FeedSubmissionProcessorTests
	{
		...
		[Test]
		public void RequestFeedSubmissionStatusesFromAmazon_WhenApiRequestThrowsException_ThenUnlockAllEntries()
		{
			var testRequestIdList = new List<string> { "submissionId1", "submissionId2", "submissionId3" };
			_marketplaceWebServiceClientMock.Setup(x => x.GetFeedSubmissionList(It.IsAny<GetFeedSubmissionListRequest>()))
				.Throws(new Exception());
			_feedSubmissionServiceMock
				.SetupSequence(fssm => fssm.FirstOrDefault(It.IsAny<Func<FeedSubmissionEntry, bool>>()))
				.Returns(new FeedSubmissionEntry() { FeedSubmissionId = "submissionId1", IsLocked = true })
				.Returns(new FeedSubmissionEntry() { FeedSubmissionId = "submissionId2", IsLocked = true })
				.Returns(new FeedSubmissionEntry() { FeedSubmissionId = "submissionId3", IsLocked = true });

			var entriesBeingUpdated = new List<FeedSubmissionEntry>();
			_feedSubmissionServiceMock
				.Setup(fssm => fssm.Update(It.IsAny<FeedSubmissionEntry>()))
				.Callback<FeedSubmissionEntry>((entryBeingUpdated) =>
				{
		...
		[Test]
		public void RequestFeedSubmissionStatusesFromAmazon_WhenApiRequestThrowsMarketplaceWebServiceException_ThenUnlockAllEntries()
		{
			var testRequestIdList = new List<string> { "submissionId1", "submissionId2", "submissionId3" };
			_marketplaceWebServiceClientMock.Setup(x => x.GetFeedSubmissionList(It.IsAny<GetFeedSubmissionListRequest>()))

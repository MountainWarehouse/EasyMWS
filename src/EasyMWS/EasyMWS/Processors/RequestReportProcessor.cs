	    ...
	    public void RequestReportFromAmazon(IReportRequestCallbackService reportRequestService, ReportRequestEntry reportRequestEntry)
	    {
		    ...
		    try
		    {
				...
				if (string.IsNullOrEmpty(reportRequestId))
			    {
				    ...
				    reportRequestEntry.LastRequested = DateTime.UtcNow;
					_logger.Warn($"AmazonMWS request succeeded for {reportRequestEntry.RegionAndTypeComputed}. But ReportRequestId was not generated by Amazon. Placing report request in retry queue. Retry count : {reportRequestEntry.RequestRetryCount}");
				}
			    else
			    {
		    ...
		    catch (MarketplaceWebServiceException e) when (IsAmazonErrorCodeNonFatal(e.ErrorCode))
		    {
			    ...
			    reportRequestEntry.LastRequested = DateTime.UtcNow;
			    reportRequestService.Update(reportRequestEntry);
				_logger.Warn($"AmazonMWS request failed for {reportRequestEntry.RegionAndTypeComputed}. Reason: ReportRequestId not generated by Amazon. Placing report request in retry queue. Retry count : {reportRequestEntry.RequestRetryCount}");
		    }
		    catch (Exception e)
		    {

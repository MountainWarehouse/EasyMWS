    ...
    internal class RequestReportProcessor : IRequestReportProcessor
    {
		    ...
		    List<(string ReportRequestId, string GeneratedReportId, string ReportProcessingStatus)> reportGenerationStatuses)
	    {
		    foreach (var reportGenerationInfo in reportGenerationStatuses)
		    {
			    ...
			    var reportFriendlyId = reportGenerationCallback.GetRegionAndTypeString();
			        ...
			        reportGenerationInfo.ReportProcessingStatus == "_DONE_NO_DATA_")
			    {
				    ...
				    _logger.Info($"Report was successfully generated by Amazon for {reportFriendlyId}. GeneratedReportId:'{reportGenerationInfo.GeneratedReportId}'.");
				}
			             ...
			             reportGenerationInfo.ReportProcessingStatus == "_IN_PROGRESS_")
			    {
				    ...
				    reportGenerationCallback.RequestRetryCount = 0;
					_reportRequestCallbackService.Update(reportGenerationCallback);
				    _logger.Info($"Report generation by Amazon is still in progress for {reportFriendlyId}.");
				}
			    else if (reportGenerationInfo.ReportProcessingStatus == "_CANCELLED_")
			    {
				    ...
				    _reportRequestCallbackService.Update(reportGenerationCallback);
				    _logger.Warn($"Report generation was cancelled by Amazon for {reportFriendlyId}. Placing report back in report request queue! Retry count is now '{reportGenerationCallback.RequestRetryCount}'");
				}
			    else
			    {
				    reportGenerationCallback.RequestReportId = null;
				    ...
				    reportGenerationCallback.RequestRetryCount++;
				    _reportRequestCallbackService.Update(reportGenerationCallback);
				    _logger.Warn($"Report status returned by amazon is {reportGenerationInfo.ReportProcessingStatus} for {reportFriendlyId}. This status is not yet handled by EasyMws. Placing report back in report request queue! Retry count is now '{reportGenerationCallback.RequestRetryCount}'");
				}
		    }
	    }

...
- A way to queue any number of Amazon MWS Reports for download from Amazon for a specified marketplaces, by only specifying the type of report, and the information required by that specific report type, along with the required seller credentials. _*only a small number of reports have been implemented / live tested so far, support for more reports will be added later on. But in theory any report type will be supported. This is still an alpha version. ** if you urgently require support for a speciffic report, a fork of the repository can be created containing the new implementation, followed pull request_
	
- Simplify the whole lifecycle of requesting reports from Amazon to just calling a Poll() method from time to time. The recommended interval for calling Poll() is 2 minutes to in order to avoid amazon request throttling.  _*there are still things to be improved around this area, the 2 minutes value is based on the average restore rate of the request quota.This is still alpha version. For more info see [Amazon MWS Throttling](https://docs.developer.amazonservices.com/en_UK/dev_guide/DG_Throttling.html)_
...
- Submit already generated feed files to Amazon, and drive the submission lifecycle by only calling a Poll() method.  _*This library does not also handle the generation of feed content. The user has to provide the feed content to submit to Amazon using the EasyMWS client._
...
_Basic usage example_ : 
  ...
  public void Main(object[] arguments) {
		...
		/// <summary>
		/// A method that calls the client.Poll() method every 2 minutes.
  ...
  ```
  _Note:  in the example above a C#7 named tuple is included in the callbackData, but callbackData can be any object._

## Client instantiation
The following parameters are required in order to create a new EasyMwsClient instance :

  _Please note that this data is required in order to request reports associated to an amazon seller account, or submit feeds associated to it._
...
- IEasyMwsLogger (interface, optional) - An EasyMwsLogger instance can be provided, in order to gain access to logs.
  - EasyMwsLogger exposes the LogAvailable event that has to be subscribed to, in order to get the logs.
...
- **_EasyMwsClient.QueueReport() method_** - adds a new report request to an internal queue.
  - _Parameters_ :
  - _ReportRequestPropertiesContainer (required)_ - an object that encapsulates the information necessary to describe a report request.
    ...
    - Using an EasyMWS report request factory to get the properties container for a report type has the advantage of only requiring information that is truly relevant for each report type (data like StartDate, EndDate, MarketplaceIdList, so on).

  - _callbackMethod (required)_ - the name of a static method with the following signature should be provided for this argument: 
...
Each time the Poll() method is called the following sequence of steps is executed (roughly) :
  ...
  - **Warning**: If this method is called too often, it Can and Will trigger MWS request throttling. At the moment the recommendation would be to call the Poll() method every 2 minutes in order to drive the lifecycle of requesting reports / submitting feeds. This could alternately be called from a system that is aware of the [Amazon throttling mechanism](https://docs.developer.amazonservices.com/en_UK/dev_guide/DG_Throttling.html), and as such take advantage of things like the throttling burst rate. If you need extra client options exposed in order to call EasyMwsClient from such a system, please create an issue for that.
...
- **_MwsMarketplaceGroup class_**
  - Some report types support the MarketplaceIdList optional parameter (EU/NA only), which specifies what marketplaces to generate a report for.
  - But all the marketplaces included in a MarketplaceIdList have to belong to the same region, a report cannot be requested for both Canada and France in the same request because they belong to [different endpoints](https://docs.developer.amazonservices.com/en_US/dev_guide/DG_Endpoints.html).
  ...
  2. Use one of TryAddMarketplace / AddMarketplace methods on a group to attempt adding additional marketplaces to that group. This methods throw InvalidOperationException if attempting to add a marketplace with a different MWS endpoint than the one already associated to the group.
